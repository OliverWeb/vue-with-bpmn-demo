...
BpmnConnectSnapping.$inject = ["eventBus", "rules"];
...
function snapTargetMidOnCenter(event, target) {
  var isCenter = every(AXES, function(axis) {
    ...
    return (
      coordinate > target[axis] + TARGET_CENTER_PADDING &&
      coordinate <
        target[axis] + matchingTargetDimension - TARGET_CENTER_PADDING
    );
  });
